...
import com.whl.banner.indicator.AbsIndicatorView;
import com.whl.banner.pageanimator.PageAnimator;
...
public class Banner extends FrameLayout {
    ...
    private AbsIndicatorView mIndicatorView;
    private int mIndicatorViewTopSpace = 0;
    private boolean hasIndicatorView = false;
    ...
    private void init() {
        ...
        mPageAnimator = new com.whl.banner.pageanimator.PageAnimator();
    }
    ...
    private void changePage(int totalScroll) {
        if (Math.abs(totalScroll) >= switchPageDis) {//切换 page
            ...
            ViewCompat.offsetLeftAndRight(mHolders.get(currentPageIdx).itemView, currentScrollPos * flag);//当前 View 滑动
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int indicatorViewHeight = 0;
        if (hasIndicatorView) {
            indicatorViewHeight = getChildAt(3).getMeasuredHeight();
        }
        ...
        final int height = chooseSize(heightMeasureSpec, (childHeight + (int) (childHeight * mScaleValue)) + getPaddingTop() + getPaddingBottom() + indicatorViewHeight + mIndicatorViewTopSpace, ViewCompat.getMinimumHeight(this) + getPaddingTop() + getPaddingBottom());
        setMeasuredDimension(width, height);
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (changed) {
            ...
            int indicatorViewWidth = 0;
            int indicatorViewHeight = 0;
            View indicatorView = null;
            if (hasIndicatorView) {
                indicatorView = getChildAt(3);
                indicatorViewWidth = indicatorView.getMeasuredWidth();
                indicatorViewHeight = indicatorView.getMeasuredHeight();
            }

            int childViewTotalHeight = childHeight + (int) (childHeight * mScaleValue) + indicatorViewHeight + mIndicatorViewTopSpace;

            int left = measuredWidth / 2 - childWidth / 2;
            int top = measuredHeight / 2 - childViewTotalHeight / 2 + getPaddingTop();
            ...
            childView2.layout(left, top, right, bottom);

            if (hasIndicatorView) {
                left = measuredWidth / 2 - indicatorViewWidth / 2;
                top = bottom + mIndicatorViewTopSpace + (int) (childHeight * mScaleValue);
                right = left + indicatorViewWidth;
                bottom = top + indicatorViewHeight;
    ...
    private boolean canLoop() {
        boolean canLoop = mIsAutoLoop && mAdapter != null && mAdapter.getItemCount() >= 3;
        return canLoop;
    }
    ...
    public Banner setIndicatorView(AbsIndicatorView indicatorView) {
        if (indicatorView != null) {
            mIndicatorView = indicatorView;
            hasIndicatorView = true;
        }
        return this;
    }

    /**
     * @param indicatorViewTopSpace px value
     * @return
     */
    public Banner setIndicatorViewTopSpace(int indicatorViewTopSpace) {
        mIndicatorViewTopSpace = indicatorViewTopSpace;
        return this;
    }
    ...
    public Banner setScaleValue(float scaleValue) {
    ...
    private static class BannerSavedState extends BaseSavedState {
        ...
        public static final Parcelable.Creator<BannerSavedState> CREATOR = new Parcelable.Creator<BannerSavedState>() {
            @Override
